...
OUT_PATH = "C:\\repos\\kaggle_fun\\Titanic\\ProcessedData"
TRAIN_DATA = "C:\\repos\\kaggle_fun\\Titanic\\RawData\\train.csv"
TEST_DATA = "C:\\repos\\kaggle_fun\\Titanic\\RawData\\test.csv"
...
def process_dataframe(df):
    ...
    #Make a column for the sum of SibSp and Parch
    df["SumFamily"] = df["SibSp"] + df["Parch"]

    #Make a column "OnOwn"
    df["OnOwn"] = 0.0
    df.loc[df["SumFamily"] > 0, ["OnOwn"]] = 1.0
    
    #Males on own unlikely to survive
    df["MaleOnOwn"] = 0.0
    df.loc[(df["OnOwn"] ==1) & (df["Sex"] == "male"), ["MaleOnOwn"]] = 1.0
    ...
    df["ScaledSumFamily"]  = 0.0
    df["ScaledSumFamily"] = df["SumFamily"] / df["SumFamily"].max()
...
if __name__ == "__main__":
    ...
    base_train_df = train_df[["PassengerId", "Survived", "ScaledClass", "ScaledSex", "AgeRecorded", "ScaledFare", "ScaledParch", "ScaledSibSp", "ScaledCabin", "ScaledNumberOfCabins", "ScaledEmbarked", "ScaledSumFamily", "OnOwn", "MaleOnOwn"]]


    file_path = os.path.join(OUT_PATH, "train_data_added_features_1.csv")
    ...
    base_test_df = test_df[["PassengerId","ScaledClass", "ScaledSex", "AgeRecorded", "ScaledFare", "ScaledParch", "ScaledSibSp", "ScaledCabin", "ScaledNumberOfCabins", "ScaledEmbarked", "ScaledSumFamily", "OnOwn","MaleOnOwn"]]

    print(base_test_df.columns[base_test_df.isna().any()].tolist())

    file_path = os.path.join(OUT_PATH, "test_data_added_features_1.csv")
